...
def handle_start (message):
    ...
    bot.send_message(message.from_user.id, 'Seja, Bem vindo {} ! ao melhor Bot de investimento.'.format(message.from_user.username), reply_markup=user_markup)

@bot.message_handler(content_types=["text"])
def handle_text (message, user_markup=None):
    if message.text == 'Iniciar':
        bot.send_message(message.from_user.id, """"SUA CONTA""" '\n'
                        '\n'
        ...
        bot.send_message(message.from_user.id, 'Por favor aguarde um minuto estamos gerando sua cartira : ')
        sleep(2)
...
(LEMBRANDO O MINIMO PARA SAQUE É DE 1500NST) """)
        """if message.text == str(Salcon) < '1500':
...
def salva(message):
    arquivo=open('oriondb.txt', 'w')
    arquivo.write(int(message.chat.id)  + message.chat.username + message.chat.text )
